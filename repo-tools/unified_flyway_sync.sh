...
publish_shared() {
    ...
    # Try subtree split first, fallback to orphan method
    echo "🔄 Creating delivery branch for $PARENT_SHARED_PATH..."
    ...
    # Try subtree split
    set +e
    git subtree split --prefix="$PARENT_SHARED_PATH" --branch "$DELIVERY_BRANCH" >/dev/null 2>&1
    RC=$?
    set -e
    
    if [[ $RC -ne 0 ]]; then
        echo "⚠️  Subtree split failed, using orphan method..."
        
        # Create orphan branch with flattened content
        git checkout --orphan "$DELIVERY_BRANCH"
        git rm -rf . 2>/dev/null || true
        
        # Copy and flatten the content
        git checkout main -- "$PARENT_SHARED_PATH"
        rsync -av "$PARENT_SHARED_PATH/" ./
        rm -rf "$PARENT_SHARED_PATH"
        
        git add -A
        git commit -m "build: export flattened $PARENT_SHARED_PATH/ contents for subtree ($(date +%Y-%m-%d))"
    else
